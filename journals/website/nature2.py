...
class journals(common_journals):
    ...
    def get(self,website,journal,url):
        ...
        wait()
        # urls=["https://www.nature.com/labinvest/volumes/96","https://www.nature.com/labinvest/volumes/95","https://www.nature.com/labinvest/volumes/94"]
        url1="https://www.nature.com/pr/volumes/68"
        ...
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "71"
                    article_info[Row_Name.YEAR] = "2012"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal,json.dumps(article_info))
        wait()
        url1="https://www.nature.com/pr/volumes/72"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "72"
                    article_info[Row_Name.YEAR] = "2012"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal,json.dumps(article_info))

        url1="https://www.nature.com/pr/volumes/73"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "73"
                    article_info[Row_Name.YEAR] = "2013"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal,json.dumps(article_info))
        url1="https://www.nature.com/pr/volumes/74"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "74"
                    article_info[Row_Name.YEAR] = "2013"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal,json.dumps(article_info))

        url1 = "https://www.nature.com/pr/volumes/75"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "75"
                    article_info[Row_Name.YEAR] = "2014"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        url1 = "https://www.nature.com/pr/volumes/76"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "76"
                    article_info[Row_Name.YEAR] = "2014"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        url1 = "https://www.nature.com/pr/volumes/77"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "77"
                    article_info[Row_Name.YEAR] = "2015"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        url1 = "https://www.nature.com/pr/volumes/78"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "78"
                    article_info[Row_Name.YEAR] = "2015"
                    article_info[Row_Name.TEMP_URL] = "https://www.nature.com" + a["href"]

                    self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        url1 = "https://www.nature.com/pr/volumes/80"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "80"
    ...
    def get_common(self,journal,url):
        return {Row_Name.ISSN:"0031-3998",Row_Name.EISSN:"1530-0447"}
