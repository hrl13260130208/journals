...
class journals(common_journals):
    ...
    def get(self,website,journal,url):
        ...
        wait()
        url1="https://www.nature.com/mp/volumes/21"
        ...
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    article_info[Row_Name.VOLUME] = "21"
        ...
        wait()
        url1="https://www.nature.com/mp/volumes/20"
        data_s = requests.get(url1)
        bs = BeautifulSoup(data_s.text, "html.parser")
        bs.extract()
        ul = bs.find("ul", id="issue-list")
        for li in ul.find_all("li"):
            ...
            if a != None:
                if a.find("h3") != None:
                    ...
                    span.extract()
                    article_info[Row_Name.ISSUE] = a.get_text()[2:].strip()
                    article_info[Row_Name.VOLUME] = "20"
        ...
        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "February 2014"
        article_info[Row_Name.ISSUE] ="2"
        article_info[Row_Name.VOLUME] = "19"
        article_info[Row_Name.YEAR] = "2014"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/19/issues/2"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "September 2013"
        article_info[Row_Name.ISSUE] = "9"
        article_info[Row_Name.VOLUME] = "18"
        article_info[Row_Name.YEAR] = "2013"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/18/issues/9"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "June 2013"
        article_info[Row_Name.ISSUE] = "6"
        article_info[Row_Name.VOLUME] = "18"
        article_info[Row_Name.YEAR] = "2013"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/18/issues/6"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "May 2011"
        article_info[Row_Name.ISSUE] = "5"
        article_info[Row_Name.VOLUME] = "16"
        article_info[Row_Name.YEAR] = "2011"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/16/issues/5"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "April 2011"
        article_info[Row_Name.ISSUE] = "4"
        article_info[Row_Name.VOLUME] = "16"
        article_info[Row_Name.YEAR] = "2011"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/16/issues/4"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))

        article_info = dict(journal_common_info)
        article_info[Row_Name.STRING_COVER_DATE] = "January 2011"
        article_info[Row_Name.ISSUE] = "1"
        article_info[Row_Name.VOLUME] = "16"
        article_info[Row_Name.YEAR] = "2011"
        article_info[Row_Name.TEMP_URL] = "https://www.nature.com/mp/volumes/16/issues/1"

        self.nm.save_journal_temp_data(journal, json.dumps(article_info))


    def get_common(self,journal,url):
        return {Row_Name.ISSN:"1359-4184",Row_Name.EISSN:"1476-5578"}
